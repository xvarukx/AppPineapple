...
namespace DAO
{
    public class Producto
    {
        ...
        static public void insertar(Entidades.Producto p)
        {
            ...
            string query = "insert into producto (idProducto, nombre, ingredienteActivo, unidad, cantidad, minimo, maximo, costo, tipo, dias,numRegistro,periodoReingreso) values(@idProducto, @nombre, @ingredienteActivo, @unidad, @cantidad, @minimo, @maximo, @costo, @tipo, @dias,@numRegistro,@periodoReingreso)";
            MySqlCommand comando = new MySqlCommand(query, Conexion.Connection);
            ...
            comando.Parameters.AddWithValue("@numRegistro", p.NumRegistro);
            comando.Parameters.AddWithValue("@periodoReingreso", p.PeriodoReingreso);
            comando.Prepare();
            comando.ExecuteNonQuery();
        ...
        static public void modificar(Entidades.Producto p)
        {
            ...
            string query = "UPDATE producto SET nombre = @nombre, ingredienteActivo = @ingredienteActivo, unidad = @unidad, cantidad = @cantidad, minimo = @minimo, maximo = @maximo, costo = @costo, tipo = @tipo, dias = @dias,numRegistro = @numRegistro, periodoReingreso = @periodoReingreso WHERE idProducto = @idProducto";
            MySqlCommand comando = new MySqlCommand(query, Conexion.Connection);
        ...
        static public DataTable mostrarProductos(string busqueda, string tipo)
        {
            ...
            Conexion.OpenConnection();
            DataTable dtDatos = new DataTable();
            string query = "select idProducto,nombre,ingredienteActivo,unidad, cantidad  from producto where tipo Like @tipo and (idProducto LIKE @busqueda or nombre LIKE @busqueda)Order by idProducto *1 ASC; ";
            MySqlCommand comando = new MySqlCommand(query, Conexion.Connection);
            comando.Parameters.AddWithValue("@tipo", "%"+tipo+"%");
            comando.Parameters.AddWithValue("@busqueda", "%"+busqueda+"%");
            comando.Prepare();
        ...
        static public void aumentarCantidadBloqueada(string id, double cantidad)
        {
            Conexion.OpenConnection();
            string query = "UPDATE producto SET cantidadBloqueada = cantidadBloqueada + @cantidad WHERE idProducto = @id";
            MySqlCommand comando = new MySqlCommand(query, Conexion.Connection);
        ...
        static public void limpiarCantidadBloqueada(string id)
        {
            Conexion.OpenConnection();
            string query = "UPDATE producto SET cantidadBloqueada = 0  WHERE idProducto = @id";
            MySqlCommand comando = new MySqlCommand(query, Conexion.Connection);
            comando.Parameters.AddWithValue("@id", id);
            comando.Prepare();
            comando.ExecuteNonQuery();
